...
const { Client } = pg;
const outPath = 'C:/Users/pc bnm/Desktop/BNM-website/actualites_verification.json';
...
try {
  ...
  console.log('WROTE_VERIFICATION', outPath);
} catch (error) {
  const payload = { error: String(error), generatedAt: new Date().toISOString() };
  fs.writeFileSync(outPath, JSON.stringify(payload, null, 2));
  console.error(error);
  process.exitCode = 1;
} finally {
