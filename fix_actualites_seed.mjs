...
const { Client } = pg;
const connectionString = 'postgresql://postgres:2026@localhost:5432/bnm_db';
...
const rows = [
  ...
  },
  {
    ...
    contenu: 'La banque Nationale de Mauritanie ouvre une agence bancaire Watani dédiée aux opérations bancaires islamiques. L’objectif de cette nouvelle agence est de mettre à la disposition des populations de Barkéwol les meilleurs services bancaires conformément à la vision de la Banque Nationale. La satisfaction clientèle et l’inclusion financière au sein de la Mauritanie resteront toujours nos objectifs finaux et nous continuerons d’ouvrer pour les atteindre.',
  ...
  },
  {
    ...
    contenu: 'La gestion des moyens de paiement est au cœur de l’activité de la Banque Nationale. Dans l’optique d’augmenter la productivité des conseillers financiers et d’améliorer l’expérience client, la Banque Nationale met en place un système pour digitaliser les demandes de cartes bancaires et de chéquiers pour une traçabilité.',
...
try {
  ...
  for (const row of rows) {
    await client.query(
      `INSERT INTO public.actualites (titre, slug, contenu, image, categorie, date_publication, created_at)
  ...
  const count = await client.query('SELECT count(*)::int AS count FROM public.actualites');
  const list = await client.query('SELECT id, titre, categorie, image FROM public.actualites ORDER BY id ASC');
  const payload = {
    tableExists: true,
  ...
  fs.writeFileSync('C:/Users/pc bnm/Desktop/BNM-website/actualites_fix_report.json', JSON.stringify(payload, null, 2));
